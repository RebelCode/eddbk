<!DOCTYPE html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>booking-str</title></head><body><div id=new-app><hello inline-template><span><h2>Instance 1: {{ selectValue }}</h2>{{ translate('Test') }}</span></hello></div><div class=next-app><hello inline-template><span><h2>Instance 2: {{ selectValue }}</h2><input type=text v-model=selectValue><vueselect :options="['foo', 'bar', 'test']" v-model=selectValue></vueselect>{{ translate('Test') }}</span></hello></div><div class=next-app><hello inline-template><span>{{ translate('Test') }}<h2>Instance 3: {{ selectValue }}</h2><vueselect :options="['foo', 'bar', 'test']" v-model=selectValue></vueselect><calendar :events="[{title: 'Sunny Out of Office',start: '2017-08-25',end: '2017-07-27'}]" locale=en></calendar></span></hello></div><script>document.addEventListener('DOMContentLoaded', function () {
      ...
      })</script><script src=https://cdnjs.cloudflare.com/ajax/libs/require.js/2.3.5/require.js></script><script type=text/javascript src=/static/js/uiFramework.js></script></body></html>